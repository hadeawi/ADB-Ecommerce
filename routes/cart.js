...
router.get('/add/:product',async function(req,res){
  ...
  await  Product.findOne({slug:slug}, async function(err,p){
    await logUserActivity(req.user._id, p._id, 'add_to_cart');
  ...
  router.get('/update/:product', async function(req,res){
    ...
    router.get('/buynow',async function(req,res){
        ...
        for(var i=0;i<p.length;i++){
            var a=(p[i].qt);  
         await   Product.findOne({slug:p[i].title}, async function(err,p1){
                if(err){
                    console.log(err);
                }
                var tt=p1.tt;
                ...
                t=tt-a;
                console.log(t);
                await logUserActivity(req.user._id, p1._id, 'purchase');
                for(var i=0;i<p.length;i++){
